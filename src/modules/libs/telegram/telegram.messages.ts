...
export const MESSAGES = {
	welcome:
		`<b>👋 Добро пожаловать в ginj Bot!</b>\n\n` +
		`Чтобы получать уведомления и улучшить ваш опыт использования платформы, давайте свяжем ваш Telegram аккаунт с ginj.\n\n` +
		`Нажмите на кнопку ниже и перейдите в раздел <b>Уведомления</b>, чтобы завершить настройку.`,
	authSuccess: `🎉 Вы успешно авторизовались и Telegram аккаунт связан с ginj!\n\n`,
	...
	follows: (user: User) =>
		`📺 <a href="https://ginj.ru/${user.username}">${user.username}</a>`,
	resetPassword: (token: string, metadata: SessionMetadata) =>
		`<b>🔒 Сброс пароля</b>\n\n` +
		`Вы запросили сброс пароля для вашей учетной записи на платформе <b>ginj</b>.\n\n` +
		`Чтобы создать новый пароль, пожалуйста, перейдите по следующей ссылке:\n\n` +
		`<b><a href="https://ginj.ru/account/recovery/${token}">Сбросить пароль</a></b>\n\n` +
		...
		`Спасибо за использование <b>ginj</b>! 🚀`,
	deactivate: (token: string, metadata: SessionMetadata) =>
		...
		`Спасибо, что пользуетесь <b>ginj</b>! Мы всегда рады видеть вас на нашей платформе и надеемся, что вы останетесь с нами. 🚀\n\n` +
		`С уважением,\n` +
		`Команда ginj`,
	accountDeleted:
		...
		`<b><a href="https://ginj.ru/account/create">Зарегистрироваться на ginj</a></b>\n\n` +
	...
	streamStart: (channel: User) =>
		...
		`Смотрите здесь: <a href="https://ginj.ru/${channel.username}">Перейти к трансляции</a>`,
	enableTwoFactor:
		`🔐 Обеспечьте свою безопасность!\n\n` +
		`Включите двухфакторную аутентификацию в <a href="https://ginj.ru/dashboard/settings">настройках аккаунта</a>.`,
	verifyChannel:
		...
		`Спасибо, что вы с нами и продолжаете развивать свой канал вместе с ginj!`
}
